...
immigrant = spark.read.format('com.github.saurfang.sas.spark')\
                 .load('s3://capstone2021data/raw/i94_immigration_data/i94_{}_sub.sas7bdat'.format(month_year))\
...
#
immigrant.write.partitionBy("i94_dt").mode("append").parquet("s3://capstone2021data/lake/immigrant/")


#
immigration = spark.read.format('com.github.saurfang.sas.spark')\
                     .load('s3://capstone2021data/raw/i94_immigration_data/i94_apr16_sub.sas7bdat')\
                     ...
                     .withColumn("i94_dt", F.lit(month_year))
immigration.write.partitionBy("i94_dt").mode("append").parquet("s3://capstone2021data/lake/immigration/")