...
immigrant = spark.read.parquet("s3://capstone2021data/lake/immigrant/")\
                      .filter("i94_dt = '{}'".format(month_year))
country_code = spark.read.parquet("s3://capstone2021data/lake/codes/country_code/")
state_code = spark.read.parquet("s3://capstone2021data/lake/codes/state_code/")
immigration = spark.read.parquet("s3://capstone2021data/lake/immigration/")\
                        .filter("i94_dt = '{}'".format(month_year))
city = spark.read.parquet("s3://capstone2021data/lake/city/")
demo = spark.read.parquet("s3://capstone2021data/lake/us_cities_demographics/")\
...
immigration_demographic.write.partitionBy("i94_dt").mode("append")\
                       .parquet("s3://capstone2021data/lake/immigration_demographic/")