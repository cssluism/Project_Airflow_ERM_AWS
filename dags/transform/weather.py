...
us_wea = spark.read.format('csv').load('s3://capstone2021data/raw/temperatures/GlobalLandTemperaturesByCity.csv', header=True, inferSchema=True)\
                                 .where(col("dt") > thres)\
...
us_wea.write.mode("overwrite").parquet("s3://capstone2021data/lake/us_cities_temperatures/")